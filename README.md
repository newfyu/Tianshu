...
可以将本机上的一个或多个文件夹变成gpt的知识库。插入base后，对话时将主要根据本地知识库中的内容来回答问题，用于构建特定领域的问答机器人。

本地知识库需要创建，在设置按钮中的“建库”中创建。创建时需要指定一个文件夹位置（文件夹中要有txt,md,pdf,docx等文本文件），然后创建即可。建库完成需要重启一次程序后才会在标签中显示新的库。或在`用户文件夹/braindoor/bases`中会看到一个同名库文件，即表示成功。
