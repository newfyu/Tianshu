...
Mac OS：
Mac上如果提示什么文件损坏，安全权限问题，在通用-隐私与安全中打开。如果使用了本地知识库，读取本地文件也会有权限允许提示。
...
加入QQ交流群797515547可以提供一些有限的测试key，但是科学上网只有自己想办法。
...
需要重启程序才在标签列表中载入新建的base，另外，目前只支持插入一个本地知识库标签，插入多个只会使用最后一个。（多知识库同时搜索我会做更多测试权衡利弊以后再加入）
...
##### Agent 智能体
规划中，用于完成一些定制好的自动化操作，以后版本挖的坑……

### 其他配置
不常用的配置可以在`用户文件夹/braindoor/config.yaml`文件中找到，可以参考[braindoor]( https://github.com/newfyu/Braindoor)的参数说明。

### 错误信息
早期版本，可能有许多环境适配问题。窗口模式下，使用cmd-shift-i可以打开开发者工具，运行日志都传入到其中，遇到问题可以先大致看下错误是什么，再反馈给我，非常感谢。
